...
export const WorkHistoryView: React.FC<WorkHistoryViewProps> = ({ db }) => {
  ...
  return (
    <div className="grid gap-3">
      <PageSection
        title="История"
        description="Журнал событий по обращениям и заявкам. Системные логи доступны администратору."
      />
      <section className="oc-card">
        <div className="oc-stream max-h-[70vh] overflow-y-auto px-3 py-2 text-[12px]">
          ...
          {events.map((ev) => (
            <div key={ev.id} className="flex gap-2 border-b border-[var(--oc-border)] py-1.5 last:border-0">
