...
export const DatabaseInspectorView: React.FC<DatabaseInspectorViewProps> = ({
  ...
  const translateCriticality = (criticality: string) => ({ LOW: 'Низкая', MEDIUM: 'Средняя', HIGH: 'Высокая', CRITICAL: 'Критическая' }[criticality] || criticality);
  const translateAssetStatus = (status: string) => ({ OK: 'В норме', WARNING: 'Предупреждение', CRITICAL_FAIL: 'Критическая неисправность' }[status] || status);

  const priorityClass = (priority: string) => `registry-badge registry-priority-${priority}`;
  const statusClass = (status: string) => `registry-badge registry-status-${status.toLowerCase()}`;
  const criticalityClass = (criticality: string) => `registry-badge registry-criticality-${criticality.toLowerCase()}`;
  const assetStatusClass = (status: string) => `registry-badge registry-asset-${status.toLowerCase()}`;
            ...
            className={`flex items-center space-x-2 px-3.5 py-2 text-xs font-bold rounded-xl transition whitespace-nowrap ${
              activeTab === 'open_tickets'
                ? isDark
                  ...
                  : 'bg-[#29263d] text-white shadow-sm font-extrabold border border-[#29263d]'
                : isDark
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg flex items-center space-x-1 transition text-xs ${
                isDark
                  ...
                  : 'bg-[#398e8b] hover:bg-[#2f7775] text-white font-extrabold shadow-sm border border-[#398e8b]'
              }`}
            >
            ...
            <tbody className="divide-y divide-slate-700/20 font-sans">
              {filteredAssets.map((asset) => (
                <tr key={asset.asset_id} className={isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'}>
                  ...
                  <td className="p-3">
                    <span className={criticalityClass(asset.criticality)}>
                      {translateCriticality(asset.criticality)}
                    </span>
                  </td>
                  <td className="p-3">
                    <span className={assetStatusClass(asset.status)}>
                      {translateAssetStatus(asset.status)}
                    </span>
                  </td>
                  <td className="p-3 text-right">
                    <button
            ...
            <tbody className="divide-y divide-slate-700/20 font-sans">
              {filteredOpenTickets.map((t) => (
                <tr key={t.ticket_id} className={isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'}>
                  ...
                  <td className="p-3"><span className={statusClass(t.status)}>{translateStatus(t.status)}</span></td>
            ...
            <tbody className="divide-y divide-slate-700/20 font-sans">
              {filteredClosedTickets.map((t) => (
                <tr key={t.ticket_id} className={isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'}>
                  ...
                  <td className="p-3"><span className={priorityClass(t.priority)}>{translatePriority(t.priority)}</span></td>
