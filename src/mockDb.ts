...
/**
 * Схема мок-базы данных приложения (React state, синхронизируется с server.ts).
 * Содержит справочники (контрагенты, объекты, оборудование, договоры)
 * и журналы заявок: открытые и закрытые (архив).
 */
export interface DatabaseSchema {
  /** Контрагенты (заказчики услуг) */
  contractors: Contractor[];
  /** Объекты клиентов (склады, производства) */
  sites: Site[];
  /** Единицы оборудования, привязанные к объектам */
  assets: Asset[];
  /** Договоры обслуживания (SLA-планы) */
  contracts: Contract[];
  /** Активные заявки */
  open_tickets: Ticket[];
  /** Закрытые заявки (архив) */
  closed_tickets: Ticket[];
}

/**
 * Стартовое наполнение БД демо-данными: три контрагента,
 * их объекты, оборудование и активные заявки.
 * Сбрасывается кнопкой «Сбросить БД» в интерфейсе.
 */
